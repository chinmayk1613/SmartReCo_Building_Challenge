...
def upgrade() -> None:
    with op.batch_alter_table("service_invocations") as batch:
        ...
        batch.create_unique_constraint("uq_service_invocations_correlation_id", ["correlation_id"])
        batch.create_unique_constraint("uq_service_invocations_langsmith_run_id", ["langsmith_run_id"])
        batch.create_index("ix_service_invocations_correlation_id", ["correlation_id"])
        ...
        batch.create_index("ix_service_invocations_langsmith_run_id", ["langsmith_run_id"])
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("service_invocations") as batch:
        ...
        batch.drop_constraint("uq_service_invocations_langsmith_run_id", type_="unique")
        batch.drop_constraint("uq_service_invocations_correlation_id", type_="unique")
